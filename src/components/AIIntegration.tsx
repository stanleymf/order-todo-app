    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center justify-between p-4 border-b">
        <div className="flex items-center gap-4">
          <Sparkles className={`h-8 w-8 ${isMobile ? "h-6 w-6" : ""}`} />
          <div>
            <h1 className={`font-bold tracking-tight ${isMobile ? "text-xl" : "text-2xl"}`}>
              AI Integration
            </h1>
            <p className="text-sm text-muted-foreground">
              Manage AI settings, training data, and the customer-facing widget.
            </p>
          </div>
        </div>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Link to="/ai-florist-widget" target="_blank">
