...
async function testOrdersAPI() {
  ...
  try {
    ...
    const pendingOrders = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders?status=pending`
    )
    console.log(`✅ Found ${pendingOrders.length} pending orders`)

    const inProgressOrders = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders?status=in_progress`
    )
    console.log(`✅ Found ${inProgressOrders.length} in-progress orders`)

    const completedOrders = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders?status=completed`
    )
    console.log(`✅ Found ${completedOrders.length} completed orders`)

    // Test 4: Filter by assignedTo
    console.log("\n4. Testing assignedTo filter...")
    const florist1Orders = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders?assignedTo=florist-1`
    )
    console.log(`✅ Found ${florist1Orders.length} orders assigned to florist-1`)

    const florist2Orders = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders?assignedTo=florist-2`
    )
    ...
    const june25Orders = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders?deliveryDate=2025-06-25`
    )
    console.log(`✅ Found ${june25Orders.length} orders for 2025-06-25`)

    // Test 6: Get single order
    console.log("\n6. Testing single order retrieval...")
    const singleOrder = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders/${order1.id}`
    )
    ...
    console.log(
      `✅ Updated order: ${updatedOrder.customerName} - ${updatedOrder.status} - ${updatedOrder.assignedTo}`
    )

    // Test 8: Verify update
    console.log("\n8. Verifying update...")
    const verifyOrder = await makeRequest(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders/${order1.id}`
    )
    ...
    const deleteResponse = await fetch(
      `${BASE_URL}/api/tenants/${TEST_TENANT_ID}/orders/${order3.id}`,
      {
        method: "DELETE",
